...
ax.text(len(categories) - 0.3, global_mean + 0.5, f'均值 {global_mean:.1f}',
        ...
        fontsize=12, color=COLORS['ref_line'], ha='right', style='italic',
    ...
    for bar, v, b in zip(bars, vals_arr, bottom):
        ...
        if v > 5:
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, b + v / 2,
                    ...
                    f'{v:.1f}%', ha='center', va='center', fontsize=12,
...
for xi, t in zip(x, totals):
    ...
    ax.text(xi, t + 1.0, f'{t:.1f}', ha='center', va='bottom', fontsize=12,
...
ax_main.set_xlabel('自变量 X', fontsize=12)

ax_main.set_ylabel('因变量 Y', fontsize=12)
...
ax_heat.set_xticklabels(labels, rotation=45, ha='right', fontsize=12)

ax_heat.set_yticks(range(n))

ax_heat.set_yticklabels(labels, fontsize=12)
    ...
    for j in range(n):
        ...
        if not mask[i, j]:
            ...
            ax_heat.text(j, i, f'{val:.2f}', ha='center', va='center',
                         ...
                         fontsize=12, color=text_color, fontweight='bold' if abs(val) > 0.7 else 'normal')
...
for i, (wedge, label, size, prev) in enumerate(zip(wedges, labels, sizes, prev_sizes)):
    ...
    ax.text(x_label, y_label,
            ...
            ha=ha, va='center', fontsize=12, color=PALETTE[i],
...
for i, (name, d) in enumerate(zip(groups, data)):
    ...
    ax.text(pos - 0.32, d.mean(), f'{d.mean():.1f}', fontsize=12, color=PALETTE[i],
...
for year, event in events.items():
    ...
    ax.text(year + 0.1, cumsum.max() * 0.95, event, fontsize=12, color=COLORS['ref_line'],
...
for i, (name, vals) in enumerate(data.items()):
    ...
    ax.text(years[-1] + 0.3, y_pos[0], f'{vals[-1]}%',
            ...
            fontsize=12, color=PALETTE[i], fontweight='bold', va='center',
...
ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12, loc='upper right')
...
ax2.text(len(items) - 0.5, 81, '80%', fontsize=12, color=COLORS['down'],
...
ax1.text(threshold_idx / 2, max(values) * 0.9, '关键少数 (80%)',
         ...
         ha='center', fontsize=12, color=PALETTE[0], style='italic',
...
ax1.set_xticklabels(items, fontsize=12, rotation=15, ha='right')

ax1.set_ylabel('频次', fontsize=12, color=PALETTE[0])

ax2.set_ylabel('累积百分比 (%)', fontsize=12, color=PALETTE[1])
...
ax1.set_ylabel('数量 (个)', fontsize=12, color=PALETTE[0])

ax2.set_ylabel('增长率 (%)', fontsize=12, color=PALETTE[1])
...
ax1.legend(lines1 + lines2, labels1 + labels2, frameon=True,
           ...
           edgecolor=COLORS['grid'], fontsize=12, loc='upper left')
...
for i, d in enumerate(data):
    ...
    ax.text(i + 0.25, d.mean(), f'{d.mean():.1f}', fontsize=12, color=PALETTE[i],
