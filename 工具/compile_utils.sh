...
if [ -f "$PAPER_DIR/论文正文.tex" ]; then
    ...
    # GMCM uses an explicit official cover and title on the abstract page;
    # do not inject a second \title or \maketitle into that route.
    if grep -q 'GMCM_COVER_START' "$PAPER_DIR/论文正文.tex" 2>/dev/null; then
        USES_TITLE_CMD=false
    fi
    ...
    if [ "$USES_TITLE_CMD" = true ]; then
        ...
        if grep -q 'GMCM_COVER_START' "$PAPER_DIR/论文正文.tex" 2>/dev/null; then
            echo "  ✓ GMCM 官方封面模板，不自动插入 \\maketitle"
        elif grep -q 'cumcmthesis\|gmcmthesis' "$PAPER_DIR/论文正文.tex" 2>/dev/null; then
            if ! grep -q '\\maketitle' "$PAPER_DIR/论文正文.tex" 2>/dev/null; then
        ...
        if grep -q 'GMCM_COVER_START' "$PAPER_DIR/论文正文.tex" 2>/dev/null; then
            echo "  ✓ GMCM 官方封面模板，跳过 \\title/\\maketitle 自动修复"
        else
