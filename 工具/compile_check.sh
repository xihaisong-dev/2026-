...
if [ -f "$PAPER_DIR/论文正文.tex" ]; then
    ...
    case "$TMPL_TYPE" in
        ...
        huawei)
            ...
            grep -q 'tableofcontents' "$PAPER_DIR/论文正文.tex" && echo "  OK: 目录命令存在" || { echo "  CRITICAL: 华为杯缺少目录"; EXIT_CODE=1; }
            grep -q 'setcounter{tocdepth}{3}' "$PAPER_DIR/论文正文.tex" && echo "  OK: 目录最多三级" || { echo "  CRITICAL: 未固定目录深度为三级"; EXIT_CODE=1; }
            ! grep -q '\\paragraph\|\\subparagraph' "$PAPER_DIR/论文正文.tex" && echo "  OK: 未发现四级目录命令" || { echo "  CRITICAL: 禁止出现四级目录命令"; EXIT_CODE=1; }
            [ -d "题目/官方论文模板" ] && find "题目/官方论文模板" -type f -print -quit | grep -q . && echo "  OK: 已上传官方论文模板" || { echo "  CRITICAL: 缺少随赛题上传的官方论文模板"; EXIT_CODE=1; }
            ;;
        *)
    ...
    grep -q '\\bibliography\|thebibliography' "$PAPER_DIR/论文正文.tex" 2>/dev/null && echo "  OK: bibliography environment present" || { echo "  FAIL: no bibliography in 论文正文.tex"; EXIT_CODE=1; }
    ...
    elif grep -q 'thebibliography' "$PAPER_DIR/论文正文.tex" 2>/dev/null; then
        echo "  OK: inline thebibliography; references.bib not required"
    else
        echo "  FAIL: references.bib not found" && EXIT_CODE=1
    fi
    if [ -f "$PAPER_DIR/论文正文.bbl" ]; then
        bibliography_entries=$(grep -c '\\bibitem' "$PAPER_DIR/论文正文.bbl" 2>/dev/null || echo 0)
    else
        bibliography_entries=$(grep -rh '\\bibitem' "$PAPER_DIR/论文正文.tex" "$PAPER_DIR"/章节/*.tex 2>/dev/null | wc -l)
    fi
    echo "  Bibliography entries in compiled source: $bibliography_entries"
    [ "$bibliography_entries" -eq 0 ] && echo "  FAIL: bibliography is empty in compiled source" && EXIT_CODE=1
fi
