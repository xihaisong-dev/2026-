...
OUT=ROOT/'图表/runs/20260924-A-q2-reserve-r06-v2'
...
def freeze():
    ...
    sources={**old['sources'],**{n:sha((ROOT/'程序'/n).read_bytes()) for n in ['q2_reserve_gate.py','q2_reserve_campaign_v2.py']}}
    ...
    for k in c['cores']:
        ...
        for arm in (['routed','reserve'] if (i+k)%2 else ['reserve','routed']):
            ...
            if arm=='routed':
                ...
                with gzip.open(old/'evaluation.json.gz','rt',encoding='utf-8') as f:assert json.loads(json.dumps(result))==json.load(f)
