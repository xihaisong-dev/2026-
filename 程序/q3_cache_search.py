...
def candidates(g,plan,result,family,budget,settings,delay):
    if family=='control':
    ...
    d=trace(g.raw,plan,result,settings,delay)
    ...
    for e in d['misses'][:64]:
        ...
        for begin,end in ws:
            ...
            for p,at in insert(plan,s,owner[s],target_group):
                proposals.append((p,dict(kind='fifo_window',group=s,position=at,tensor=e['tensor'],window=[begin,end],old_read=e['time'],critical=e['critical'])))
    for p,move in proposals:
        add(p,move)
