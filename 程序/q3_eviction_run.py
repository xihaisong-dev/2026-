...
from q3_eviction import solve
from q3_run import dump_gz
MODES=['consumer','source','joint']
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--baseline',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--workers',type=int,default=12);p.add_argument('--selection',type=Path,required=True);a=p.parse_args();selection=json.loads(a.selection.read_text());DISCOVERY=selection['discovery'];VALIDATION=selection['validation'];a.output.mkdir(parents=True,exist_ok=False)
    ...
    write_json(a.output/'contract.json',dict(discovery=DISCOVERY,validation=VALIDATION,modes=MODES,budget=24,batch=4,baseline_hashes=hashes,sources={x:sha((ROOT/'程序'/x).read_bytes()) for x in ['q3_eviction.py','q3_eviction_run.py','q3_eviction_screen.py','q2_bottleneck_j.py']},promotion='No expansion unless joint improves >=3/4 validation cases, mean reduction>=0.2%, >=2 validation wins vs each single-move control and larger mean reduction than both. All official and FIFO/physical audits pass. 24 distinct candidates maximum per method, fixed anchor for clean ablation; no untested learned selection after discovery. Cases chosen by frozen-trace mechanism and legal proposal availability only, before optimization outcomes.',selection=selection,scope='New algorithm validation cases, not unseen baseline data. Parameter studies retained unchanged. NOT formal stage approval.'))
    rows=[];errors=[]
