...
from q3_asap import solve as old_solve
from q3_portfolio import solve as new_solve
def solve(raw,seed,s,d,c,mode,budget):
    return old_solve(raw,seed,s,d,c,'beam',budget) if mode=='beam' else new_solve(raw,seed,s,d,c,mode,budget)
from q3_run import dump_gz
DISCOVERY=[(49,4),(58,2),(67,2),(98,4)]
VALIDATION=[(21,5),(36,3),(53,4),(89,2)]
MODES=['beam','reserved','fixed','adaptive']
...
def main():
    ...
    write_json(a.output/'contract.json',dict(discovery=DISCOVERY,validation=VALIDATION,modes=MODES,budget=24,batch=4,baseline_hashes=hashes,sources={x:sha((ROOT/'程序'/x).read_bytes()) for x in ['q3_asap.py','q3_asap_fast.py','q3_portfolio.py','q3_portfolio_run.py','q3_cache_search_fast.py','q2_bottleneck_j.py']},promotion='No automatic expansion. Adaptive must win >=2/4 new validation cases vs baseline, mean reduction >=0.2%, win >=2/4 vs beam, and exceed mean reduction of beam, reserved and fixed. Actual calls and wall time reported; 24 unique candidate cap in all modes. Root reserve16; remaining calls alternate root/best and gain-based family allocation with every fourth exploratory call. All audits required.',scope='New algorithm validation cases, not unseen baseline data. Parameter studies retained unchanged. NOT formal stage approval.'))
    rows=[];errors=[]
