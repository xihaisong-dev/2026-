...
def run(out):
    ...
    for n in [72]:
        ...
        owners={v:owners[mapping[v]] for v in g.order};mapping={v:v for v in g.order}
        outputs={}
        for repeat in range(2):
            for arm in (['base','compiled_uncached','prepared'] if repeat%2==0 else ['prepared','compiled_uncached','base']):
                cls=GenerationReuse if arm=='base' else OrderingPreparation
                plans=[];times=[];t=time.perf_counter()
                with (cls(g,max_bytes=0) if arm=='compiled_uncached' else cls(g)) as reuse:
                    for i in range(4):
        ...
        audit.append(dict(case=n,official=False,scope='stress ordering equivalence only; not a selected solution'));save()
    summary=[]
    import statistics
    for n in [72]:
        a=[x['seconds'] for x in rows if x['case']==n and x['arm']=='base'];b=[x['seconds'] for x in rows if x['case']==n and x['arm']=='prepared']
        z=[x['seconds'] for x in rows if x['case']==n and x['arm']=='compiled_uncached']
        summary.append(dict(case=n,base_median=statistics.median(a),uncached_median=statistics.median(z),prepared_median=statistics.median(b),reduction_pct=100*(1-statistics.median(b)/statistics.median(a)),reuse_only_pct=100*(1-statistics.median(b)/statistics.median(z))))
