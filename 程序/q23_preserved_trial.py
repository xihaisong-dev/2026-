...
def worker(args,deadline):
    ...
    try:
        ...
        def assess(p,label):
            ...
            return True
        prefix_complete=True;prefix_verify_seconds=0
        ...
        if prefix_complete and time.monotonic()+1.5*last<search_end:
            # A slow optional diagnosis must not discard the completed baseline.
            # Both arms pay for the same verified phase-boundary checkpoint.
            t=time.monotonic();verify(best,result);checkpoint(best,result,'base_complete')
            prefix_verify_seconds=time.monotonic()-t
        ...
        atomic(out/'details.json',dict(anchor=objective(anchor),selected=objective(result),generation_seconds=generation_seconds,anchor_verify_seconds=anchor_seconds,prefix_verify_seconds=prefix_verify_seconds,final_verify_seconds=time.monotonic()-t,reserved_seconds=reserve,evaluations=log,diagnostic=diagnostic,protected_keys=protected_keys,prefix_complete=prefix_complete,final_replay_complete=True))
