...
def solve_experimental(raw, settings, waits, cores=4, evaluation_budget=12, seed=0,
    ...
    if features & {'event_seed_rank','preparation_reuse'} and 'event_opportunity' not in features:
        raise ValueError('event ranking/reuse require event opportunity')
    if 'event_opportunity' in features and ('bounded_routes' not in features or 'opportunity_gate' in features):
        raise ValueError('event_opportunity requires bounded_routes and excludes local gate')
    if 'opportunity_gate' in features and 'bounded_routes' not in features:
        raise ValueError('opportunity_gate requires bounded_routes')
    if 'bounded_routes' in features and ('component_local_rank' not in features or features & {'memory_route','hybrid_route'}):
        raise ValueError('bounded_routes requires local rank and excludes separate routes')
    ...
    shared_ranker = None
    if 'preparation_reuse' in features:
        from q1_exact_placement import PartitionEvaluator
        from q1_local_rank import LocalRank
        from q1_fast_evaluator import evaluator as counter_engine
        shared = PartitionEvaluator(raw, g.bandwidth, g.capacity, g.cross, g.same,
                                    backend_engine=counter_engine() if evaluator_backend=='counter' else None,
                                    cache_on_evaluate=False)
        shared_ranker = LocalRank(raw,g,event=True,preparer=shared,reuse_scores=True)
    ...
    def evaluate(plan, label, observe_improving_only=False):
        ...
        def compute():
            if shared_ranker is not None:
                return shared_ranker.preparer.evaluate(plan)
    ...
    if structural and cores > 1:
        ...
        if 'component_guard' in features:
            ...
            if features & {'memory_route','hybrid_route','bounded_routes'}:
                from q1_memory_routes import routed_candidates
                route = 'auto' if 'bounded_routes' in features else ('memory' if 'memory_route' in features else 'hybrid')
                options = {}
                if 'event_seed_rank' in features or shared_ranker is not None:
                    options = dict(ranking='event' if 'event_seed_rank' in features else 'local', ranker=shared_ranker)
                proposals, structural_info = routed_candidates(raw, settings, waits, cores, route, **options)
    ...
    for attempt in range(evaluation_budget * 80 if cores > 1 else 0):
        ...
        if pending_component and (ordinary_random or duplicate_opportunity):
            ...
            record = {'candidate': 'structural_' + kind, 'replaces_candidate': label,
                      'replaced_plan_sha256': hashlib.sha256(json.dumps(trial, sort_keys=True).encode()).hexdigest(),
                      'replaced_exact_evaluation_id': cache.get(json.dumps(trial, sort_keys=True)),
                      'replacement_at_evaluation': len(history),
                      'incumbent_plan_sha256': hashlib.sha256(json.dumps(best[1], sort_keys=True).encode()).hexdigest()}
            ...
            if canonical_key(component_plan) in known:
                ...
                allowed = True
                if 'opportunity_gate' in features and structural_info.get('route'):
                    from q1_opportunity import compare_local_profiles
                    prediction = next(x['local_prediction'] for x in structural_info['ranked'] if x['candidate'] == kind)
                    allowed, decision = compare_local_profiles(g, prediction, best[1], best[2])
                    record['opportunity_comparison'] = decision
                    if not allowed:
                        record['status'] = 'screened_no_predicted_gain'
                if 'event_opportunity' in features and structural_info.get('route'):
                    from q1_opportunity import compare_event_profiles
                    allowed, decision = (compare_event_profiles(raw, g, component_plan, best[1], shared_ranker)
                                         if shared_ranker is not None else compare_event_profiles(raw, g, component_plan, best[1]))
                    record['event_comparison'] = decision
                    if not allowed:
                        record['status'] = 'screened_no_event_gain'
                if allowed:
    ...
    stats = {'method': 'experimental', 'features': sorted(features), 'seed': seed,
             ...
             'preparation_reuse_stats': shared_ranker.stats() if shared_ranker else None,
