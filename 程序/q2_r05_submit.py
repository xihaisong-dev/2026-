"""Reproduce r05 from a graph, optionally reusing a fixed migration seed."""
...
def main():
    ...
    ap.add_argument('--decision',type=Path,default=ROOT/'图表/runs/20260924-A-q2-full-r05/summary.json')
    a=ap.parse_args()
    ...
    assert decision['status']=='full100_evaluated'
    arm=decision['selected_global_algorithm'];assert arm in {'legacy','routed'}
    ...
    if a.cores==1:
        ...
        plan,result,stats=solve(raw,settings,delay,provenance,a.cores,migration,arm)
    ...
    write_json(a.output/'run.json',dict(problem=2,algorithm=arm,cores=a.cores,makespan=result['makespan'],
               ...
               input_sha256=sha(a.graph.read_bytes()),decision_sha256=sha(a.decision.read_bytes()),
