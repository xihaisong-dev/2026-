"""Frozen r03 protocol: bottleneck classification x iteration ablation."""
import argparse,json,time
from pathlib import Path
from concurrent.futures import ProcessPoolExecutor,as_completed
from q1_io import ROOT,PROCESSED,sha,write_json
from q3_solver import load
from q3_adaptive import solve
from q3_run import dump_gz
DISCOVERY=[(5,5),(12,5),(67,2),(84,2),(92,3),(72,4)]
VALIDATION=[(9,5),(27,3),(41,4),(58,2),(81,5),(95,4)]
MODES=['single','iterative','unclassified_iterative']
...
def job(args):
    ...
    row=dict(case=case,cores=n,mode=mode,baseline=anchor['makespan'],makespan=result['makespan'],ratio=anchor['makespan']/result['makespan'],added_bytes=result['data_movement_bytes']['added_copy_bytes'],hit_rate=result['cache_stats']['hit_rate'],calls=len(search['evaluations']),diagnoses=len(search['rounds']),accepted_rounds=sum(x['accepted'] for x in search['rounds']),invalid=sum('error' in x for x in search['evaluations']),seconds=time.perf_counter()-t)
...
def main():
    ...
    write_json(a.output/'contract.json',dict(discovery=DISCOVERY,validation=VALIDATION,modes=MODES,budget=24,batch=4,baseline_hashes=hashes,sources={x:sha((ROOT/'程序'/x).read_bytes()) for x in ['q3_adaptive.py','q3_adaptive_run.py','q3_cache_search_fast.py','q3_cache_search.py','q3_explore.py']},promotion='Classified iterative: >=3/6 discovery wins, mean ratio>=1.005; >=3/6 new validation wins, mean ratio>=1.002; validation mean ratio strictly exceeds both ablations, >=2 validation wins vs single, all audits pass. Otherwise no full expansion.',scope='New algorithm validation cases, not unseen baseline data. Parameter studies retained unchanged. NOT formal stage approval.'))
    rows=[];errors=[]
