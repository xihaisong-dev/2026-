...
def audit(root,n,k):
    ...
    for q in (1,2,3):
        target=out/f'q{q}'
        if q==1:cmd=[sys.executable,'程序/q1_submit.py',str(graph),'-n',str(k),'--output',str(target),'--verify-final']
        else:
        ...
        row=dict(problem=q,case=n,cores=k,command=cmd,returncode=p.returncode,timeout=timeout,standalone_stage_seconds=elapsed,graph_only_cumulative_seconds=cumulative,within_600_seconds=cumulative<=600,verified=p.returncode==0 and plan.exists(),host=platform.node())
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--cases',nargs='+',type=int,default=[1,14,48,67,72,76]);p.add_argument('--cores',type=int,default=5);p.add_argument('--workers',type=int,default=2);a=p.parse_args()
    a.root.mkdir(parents=True,exist_ok=False)
    save(a.root/'contract.json',dict(cases=a.cases,cores=a.cores,workers=a.workers,selection='predeclared representative, dependency-heavy and largest input graphs; not all-100 cold coverage',source_sha256={x.name:sha(x) for x in Path('程序').glob('*.py')}))
    with concurrent.futures.ThreadPoolExecutor(max_workers=a.workers) as ex:
        fs=[ex.submit(audit,a.root,n,a.cores) for n in a.cases]
