        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            for q in [2,3]:
                ...
                for reuse in [False,True]:
                    out=p/f'{q}_{reuse}'
                    a=dict(problem=q,graph=str(p/'raw.json'),output=str(out),migration=str(p/'seed.json'),seed_plan=str(p/'seed.json'),anchor=None,reuse=reuse,seconds=30,cores=5,seed=0,max_proposals=40,cache_mib=32)
                    r=run(a)
