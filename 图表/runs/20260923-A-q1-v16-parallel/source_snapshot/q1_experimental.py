...
EXTRA_FEATURES = frozenset({'insertion', 'comm_rank', 'lookahead', 'calibrated', 'joint', 'budget_adapt', 'ddr', 'guarded_joint', 'beam', 'partition_guard', 'shared_input', 'local_repair', 'repair_move', 'region', 'region_gap', 'fluid_rank', 'boundary_refine', 'exact_region', 'wide_region', 'uphill_region', 'phase_rank'})
...
def solve_experimental(raw, settings, waits, cores=4, evaluation_budget=12, seed=0,
    ...
    if features & {'wide_region', 'uphill_region', 'phase_rank'} and 'exact_region' not in features:
        ...
        try:
            if arm == 'grain':
                ...
                try:
                    ...
                    continue
                label += '_' + kind
            elif arm == 'random' and 'region' in features and pulls['random'] >= 2 and region_attempts < 1:
                ...
                if 'region' not in queues or queues['region'][0] != current_key:
                    if 'exact_region' in features:
                        ...
                        queue = ranked_candidates(g, raw, best[1], best[2], cores, 'wide_region' in features, 'uphill_region' in features, 'phase_rank' in features)
