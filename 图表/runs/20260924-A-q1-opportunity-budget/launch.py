...
protocol='审查/问题一候选机会成本验证协议_20260924.md'
spec=json.loads(Path('审查/问题一候选机会成本验证协议_20260924.json').read_text())
...
s=run('development',spec['development'],['routes_guarded'])
...
for name,cases in [('component_memory',['case_058','case_039','case_072']),('component_hybrid',['case_009','case_100','case_040'])]:
 before=sum(r['makespan'] for r in json.loads((prior/name/'summary.json').read_text())['runs'] if r['case'] in cases)
...
if proceed:run('extension',spec['extension'],['routes_unguarded','routes_guarded'])
