...
def paired(rows, baseline):
    ...
    timed=[b['solve_seconds'] for a,b in differences if b.get('solve_timing_complete',True) and b.get('solve_seconds') is not None]
    return dict(count=len(rows),wins=sum(b['makespan']<a['makespan'] for a,b in differences),
                ...
                baseline_partition_added_bytes=sum(a['partition_added_copy_bytes'] for a,b in differences),
                candidate_partition_added_bytes=sum(b['partition_added_copy_bytes'] for a,b in differences),
                baseline_spill_added_bytes=sum(a['spill_added_copy_bytes'] for a,b in differences),
                candidate_spill_added_bytes=sum(b['spill_added_copy_bytes'] for a,b in differences),
                ...
                timed_count=len(timed),untimed_count=len(differences)-len(timed),
                mean_solve_seconds=statistics.mean(timed) if timed else None,
                median_solve_seconds=statistics.median(timed) if timed else None,
                max_solve_seconds=max(timed) if timed else None,
                over_600=sum(t>600 for t in timed))
...
def analyze(folder):
    ...
    result=dict(summary_sha256=sha((folder/'summary.json').read_bytes()),count=len(rows),
                wall_seconds=summary['wall_seconds'],execution_segments=summary.get('execution_segments',[]),variants={},validation={},regressions=[],
    ...
    result['timing_note']='Mixed local/server concurrency; no wall-clock speedup claim. Four recovered searches lack outer stopwatch.'
...
def main():
    ...
    for name in VARIANTS:
        a=first['variants'][name]['all'];lines.append(f"| {name} | {a['timed_count']}/{a['count']} | {a['median_solve_seconds']:.2f} | {a['max_solve_seconds']:.2f} | {a['over_600']} |")
    lines+=['','## 组合退步点','']
    ...
    for name in VARIANTS:
        a=first['variants'][name]['all']
        lines.append(f"| {name} | {a['candidate_partition_added_bytes']} | {a['candidate_spill_added_bytes']} |")
    lines+=['',
