...
def worker(job):
    case, cores, seed, name, output, sources, reuse_roots = job
    ...
    start=time.perf_counter()
    from q1_verified_replay import reuse_verified
    reused=reuse_verified(reuse_roots,case,cores,folder/'plan.json',result)
    if reused:
        actual=result
        write_json(folder/'verification_reuse.json',reused[1])
    else:
    ...
    row=dict(case=case,cores=cores,seed=seed,config=name,makespan=result['makespan'],
             ...
             fixed_reference_hits=stats['cache_hits'],verification_calls=0 if reused else 1,verification_reused=bool(reused),verification_passed=True,
...
def main():
    ...
    ap.add_argument('--variants',nargs='+',choices=sorted(CONFIGS),default=None)
    ap.add_argument('--cores',nargs='+',type=int,choices=[2,3,4,5],default=None)
    ap.add_argument('--seeds',nargs='+',type=int,default=None)
    ap.add_argument('--protocol',type=Path,default=ROOT/'审查/问题一结构初解同预算实验协议_20260923.md')
    ap.add_argument('--robustness',action='store_true',help='Predeclared seeds 1,2; baseline vs combined on 5 cores')
    ap.add_argument('--verified-plan-runs',nargs='*',default=[])
    args=ap.parse_args()
    ...
    for name in args.verified_plan_runs:
        reused_summary=json.loads((Path(name)/'summary.json').read_text(encoding='utf8'))
        assert reused_summary['source_zip_sha256']==manifest['source_sha256']
    ...
    variants=args.variants or (['shared_region','seed_combined'] if args.robustness else VARIANTS)
    cores=args.cores or ([5] if args.robustness else [2,3,4,5])
    seeds=args.seeds or ([1,2] if args.robustness else [0])
    if any(len(x)!=len(set(x)) for x in [cases,variants,cores,seeds]):raise ValueError('Duplicate jobs in requested matrix')
    singles={}
    for case in cases:
    ...
    product=itertools.product(cases,cores,seeds,variants)
    jobs=[(case,n,seed,variant,str(args.output.resolve()),sources,args.verified_plan_runs) for case,n,seed,variant in product]
    protocol=args.protocol
    summary=dict(verified_plan_runs=args.verified_plan_runs,completed=False,command=sys.argv,python=platform.python_version(),workers=args.workers,
