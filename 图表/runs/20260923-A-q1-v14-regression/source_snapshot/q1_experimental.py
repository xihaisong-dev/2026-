...
EXTRA_FEATURES = frozenset({'insertion', 'comm_rank', 'lookahead', 'calibrated', 'joint', 'budget_adapt', 'ddr', 'guarded_joint', 'beam', 'partition_guard', 'shared_input', 'local_repair', 'repair_move', 'region', 'region_gap', 'fluid_rank', 'boundary_refine'})
        ...
        try:
            if arm == 'grain':
                ...
                if 'region' not in queues or queues['region'][0] != current_key:
                    if features & {'fluid_rank', 'boundary_refine'}:
