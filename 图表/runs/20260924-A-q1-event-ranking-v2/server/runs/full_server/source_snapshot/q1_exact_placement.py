...
class PartitionEvaluator:
    def __init__(self, raw, bandwidth, capacity, cross, same, max_partitions=2, backend_engine=None, cache_on_evaluate=True):
        ...
        self.cache_on_evaluate = cache_on_evaluate
    ...
    def _build(self, raw, plan, bandwidth, capacity):
        ...
        if key not in self.cache:
            ...
            if self.in_evaluation and not self.cache_on_evaluate:
                # One-off global candidates must not evict prepared ranking
                # candidates or pay for a deep copy that will never be reused.
            ...
            # The original build is already private to this caller. Only the
            # stored copy needs isolation; do not copy it back on a miss.
            return built
        else:
            self.hits += 1
