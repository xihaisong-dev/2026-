...
def guarded_component_candidate(raw, settings, waits, cores, *, max_candidates=1, ranking="static", ranker=None):
    ...
    """
    if ranking not in ("static", "local", "event"):
    ...
    if ranking in ('local', 'event'):
        ...
        for item in ranked:
            ...
            item[2]['local_prediction'] = ranker.last_components.get('local', value)
            if ranker.event:
                item[2]['event_prediction'] = value
        ranked.sort(key=lambda x: (x[2]['event_prediction' if ranking=='event' else 'local_prediction'], x[1]))
        info['ranking'] = 'event_profiles_frozen_state' if ranking=='event' else 'local_task_profiles_frozen_state'
        info['local_preparation'] = ranker.stats()
        info['ranking_global_evaluations'] = ranker.preparer.evaluations-global_before
        assert info['ranking_global_evaluations'] == 0
