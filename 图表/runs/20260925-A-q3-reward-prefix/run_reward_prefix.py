...
root=pathlib.Path('图表/runs/20260925-A-q3-reward-prefix'); c=json.loads((root/'contract.json').read_text())
...
def job(q,n,arm):
 ...
 cmd=['/home/xihs/q1-runtime/run-python','程序/q23_reward_prefix.py','--problem',str(q),'--graph',f'数据/processed/q1/data/case_{n:03d}.json','--output',str(out),'--seconds','590','--cores','5','--seed','0','--max-proposals','384']
 ...
 cmd+=['--policy','reward' if arm in ['reward','prepared'] else arm]
 if arm=='prepared':cmd+=['--ordering-prepare']
 ...
 return row
jobs=[(q,n,a) for q in [int(k) for k in c['cases']] for n in sorted(c['cases'][str(q)],reverse=True) for a in c['arms']]
