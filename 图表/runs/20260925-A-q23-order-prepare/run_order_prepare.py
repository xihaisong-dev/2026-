...
root=pathlib.Path('图表/runs/20260925-A-q23-order-prepare'); c=json.loads((root/'contract.json').read_text())
...
def job(q,n,arm):
 ...
 cmd=['/home/xihs/q1-runtime/run-python','程序/q23_order_portfolio.py','--problem',str(q),'--graph',f'数据/processed/q1/data/case_{n:03d}.json','--output',str(out),'--seconds','590','--cores','5','--seed','0','--max-proposals','96']
...
with concurrent.futures.ThreadPoolExecutor(max_workers=min(4,max(1,resource.get('recommended_workers',12)))) as ex:
