...
EXTRA_FEATURES = frozenset({'insertion', 'comm_rank', 'lookahead', 'calibrated', 'joint', 'budget_adapt', 'ddr', 'guarded_joint', 'beam', 'partition_guard', 'shared_input', 'local_repair', 'repair_move', 'region', 'region_gap', 'fluid_rank', 'boundary_refine', 'exact_region', 'wide_region', 'uphill_region'})
...
def solve_experimental(raw, settings, waits, cores=4, evaluation_budget=12, seed=0,
    ...
    if features & {'wide_region', 'uphill_region'} and 'exact_region' not in features:
            ...
            if arm == 'grain':
                ...
                if 'region' not in queues or queues['region'][0] != current_key:
                    if 'exact_region' in features:
                        ...
                        queue = ranked_candidates(g, raw, best[1], best[2], cores, 'wide_region' in features, 'uphill_region' in features)
