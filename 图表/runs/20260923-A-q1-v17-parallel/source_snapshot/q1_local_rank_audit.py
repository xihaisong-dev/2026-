...
def main():
    ...
    for i, item in enumerate(source['runs']):
        ...
        ranker = LocalRank(raw, g, args.phase, args.event)
        rows = []
        for j, row in enumerate(item['rows']):
            ...
            rows.append({'label': row['label'], 'local_exact': replay(g,plan,duration), 'phase': score, 'event': score,
    ...
    for i, (item, output) in enumerate(zip(source['runs'], pending)):
        ...
        output['rankings'] = {f: metrics(output['rows'],f) for f in (['base','local_exact','event'] if args.event else (['base','local_exact','phase'] if args.phase else ['base','local_exact']))}
