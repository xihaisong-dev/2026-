...
def compare_event_profiles(raw, g, candidate_plan, incumbent_plan, ranker=None):
    ...
    start=time.perf_counter();ranker=ranker or LocalRank(raw,g,event=True)
    before=ranker.stats()
    ...
    stats=ranker.stats()
    assert stats['global_evaluations']==before['global_evaluations']
    return candidate<incumbent,dict(candidate_event=candidate,incumbent_event=incumbent,
        ...
        full_score_calls=0,local_preparation_calls=stats['calls']-before['calls'],
        score_cache_hits=stats['score_cache_hits']-before['score_cache_hits'],preparation=stats,
        seconds=time.perf_counter()-start)
