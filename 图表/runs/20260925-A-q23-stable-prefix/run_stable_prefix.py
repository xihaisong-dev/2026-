...
root=pathlib.Path('图表/runs/20260925-A-q23-stable-prefix'); c=json.loads((root/'contract.json').read_text())
...
def job(q,n,arm):
 ...
 cmd=['/home/xihs/q1-runtime/run-python','程序/q23_stable_prefix.py','--problem',str(q),'--graph',f'数据/processed/q1/data/case_{n:03d}.json','--output',str(out),'--seconds','590','--cores','5','--seed','0','--max-proposals','384']
 ...
 cmd+=['--policy','legacy' if arm=='legacy' else 'stable']
