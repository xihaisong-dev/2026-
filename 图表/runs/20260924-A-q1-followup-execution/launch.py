...
def main():
    manifest=verify();start=time.perf_counter();jobs=[]
    spec=[('20260924-A-q1-followup-primary',['--cases']+['case_'+x for x in ['017','045','048','065','077','002','028','063','067','085','006','040','090','097']],12),('20260924-A-q1-followup-robust',['--robustness'],4)]
    for name,extra,workers in spec:
        cmd=[sys.executable,'-u',str(ROOT/'程序/q1_seed_campaign.py'),'--reference-run',str(ROOT/'references'),'--output',str(ROOT/'图表/runs'/name),'--workers',str(workers),'--variants','component_slot','component_followup','--protocol',str(ROOT/'审查/问题一条件第二布局实验协议_20260924.md')]+extra
    ...
    state['wall_seconds']=time.perf_counter()-start;state['completed']=all(j['returncode']==0 for j in state['jobs']);write_json(ROOT/'server_execution.json',state)
    with zipfile.ZipFile(ROOT/'results.zip','w',zipfile.ZIP_DEFLATED) as z:
        for name in ['server_execution.json','launch.py','input_package.json']:z.write(ROOT/name,name)
