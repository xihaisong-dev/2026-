...
def candidates(g, plan, result, cores, gap=False, all_candidates=False, max_tasks=4):
    ...
    for count in sorted({max(1, len(region)-1), len(region), min(max_tasks, len(region)+1)}):
        if count > len(nodes):
            continue
