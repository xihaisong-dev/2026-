...
def routed_candidates(raw, settings, waits, cores, kind):
    ...
    plans, info = guarded_component_candidate(raw, settings, waits, cores,
                                               max_candidates=2, ranking='local')
    gate = info['gate']
    ...
    for i, (name, plan) in enumerate(pool):
        ...
        ranked.append((prediction, i, name, plan, bound))
    ranked.sort(key=lambda x:x[:2]); best = ranked[0]
    info.update(route=kind, ranking='local_task_profiles_frozen_state',
                ranked=[dict(candidate=x[2],local_prediction=x[0],compute_lower_bound=x[4]) for x in ranked],
