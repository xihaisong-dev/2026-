...
EXTRA_FEATURES = frozenset({'insertion', 'comm_rank', 'lookahead', 'calibrated', 'joint', 'budget_adapt', 'ddr'})
...
class CostGraph(Graph):
    ...
    def observe(self, plan, result):
        ...
        base = super().costs(mapping)[0] if self.calibrated else {}
        for s, profile in result['step3_by_task'].items():
            key = tuple(sorted(groups[int(s)]))
            value = profile['local_makespan']
            if self.calibrated and key not in self.local_observations and base[int(s)] > 0:
