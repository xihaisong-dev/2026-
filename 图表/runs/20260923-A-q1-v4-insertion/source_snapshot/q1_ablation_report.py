...
def main():
    ...
    if any(s['input_sha256'] != summaries[0]['input_sha256'] or
           s['source_zip_sha256'] != summaries[0]['source_zip_sha256'] for s in summaries):
    ...
    lines = ['# 问题一同预算消融', '',
             f"共 {len(rows)} 次运行，{output['official_evaluations']} 次官方评估。每次 12 次，包含初解。",
    ...
    lines += ['', '## 完成时间（cycles）', '', '| 图/种子 | ' + ' | '.join(configs) + ' |',
              '| --- | ' + ' | '.join(['---:']*len(configs)) + ' |']
    for case, cores, seed in sorted({(r['case'], r['cores'], r['seed']) for r in rows}):
        ...
        lines.append(f'| {case}/{seed} | ' + ' | '.join(str(values[c]) for c in configs) + ' |')
