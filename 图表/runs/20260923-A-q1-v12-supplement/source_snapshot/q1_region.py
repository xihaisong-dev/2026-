...
def rebuild(g, old, mapping, region, cores, mode=0):
    ...
    for s in topological(pred, succ, priority):
        ...
        for c in ([owner[s]] if s in exterior else range(cores)):
            start = end[seqs[c][-1]] + g.same if seqs[c] else 0
            start = max(start, max((end[a] + (g.cross if assigned[a] != c else 0)
                                     for a in pred[s]), default=0))
            choices.append((start + duration[s], c))
        end[s], c = min(choices)
        assigned[s] = c
        seqs[c].append(s)
...
def candidates(g, plan, result, cores):
    ...
    for label, trial in variants[:6]:
        ...
        for mode in range(2):
            ...
            try:
                candidate, score = rebuild(g, plan, trial, region, cores, mode)
    ...
    for score, label, candidate in sorted(pool, key=lambda x: (x[0], x[1]))[:2]:
        info['returned'] += 1
