...
EXTRA_FEATURES = frozenset({'insertion', 'comm_rank', 'lookahead'})
...
def directed_candidates(g, plan, result, cores, critical=False, adaptive=False):
    mapping = {int(u): s for u, s in plan['node_to_subgraph'].items()}
    ...
    for task in tasks[:4]:
        ...
        if len(nodes) > 1:
            ...
            for i in cuts:
                ...
                try:
                    yield 'split', g.schedule(trial, cores)[0]
                except ValueError:
                    continue
        if adaptive:
            ...
            for b in sorted(succ[s])[:2]:
                ...
                try:
                    yield 'merge', g.schedule(trial, cores)[0]
                except ValueError:
                    continue
...
def solve_experimental(raw, settings, waits, cores=4, evaluation_budget=12, seed=0,
    ...
    g = CostGraph(raw, settings, waits, 'local_cost' in features)
    ...
    def evaluate(plan, label):
        ...
        try:
            ...
            return None
        t = time.perf_counter()
        result = evaluate_scene_a(raw, plan, g.bandwidth, g.capacity, g.cross, g.same)
    ...
    if cores > 1 and len(history) < evaluation_budget:
        ...
        if len(history) < evaluation_budget:
            evaluate(g.schedule(multilevel(g, initial, cores), cores)[0], 'multilevel')
    ...
    for attempt in range(evaluation_budget * 80 if cores > 1 else 0):
        ...
        try:
            if arm == 'grain':
                    ...
                    if 'lookahead' in features:
                        ...
                        directed = chain(candidates(g, best[1], best[2], cores),
                                         directed_candidates(g, best[1], best[2], cores,
                                                             'critical' in features, 'adaptive' in features))
                    else:
                ...
                trial = parent[1]
                kind = rng.choice(['merge', 'split', 'boundary', 'migrate', 'reorder'])
