...
def ranked_candidates(g, raw, plan, result, cores, wide=False, uphill=False, phase=False, event=False, chain=False):
    frozen = FrozenCost(g, plan, result)
    ranker = LocalRank(raw, frozen.g, phase, event)
    info = {'kind': 'local_exact', 'phase': phase, 'event': event, 'chain': chain, 'wide': wide, 'uphill': uphill,
    ...
    for size in ([4,8,12] if wide else [4]):
        ...
        if len(scored) >= 6 and min(x[0] for x in scored) < incumbent_score:
            break
    if chain:
        from q1_critical_chain import candidates as chain_candidates
        frozen.g.chain_stats = []
        for label,candidate in chain_candidates(frozen.g,plan,result,cores):
            add(label,candidate)
        g.chain_stats.extend(frozen.g.chain_stats)
    ...
    info['shortlist'] = [{'label': label,'score': value} for value,label,_ in sorted(scored,key=lambda x:(x[0],x[1]))[:2]]
    frozen.check()
