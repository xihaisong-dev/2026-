...
EXTRA_FEATURES = frozenset({'insertion', 'comm_rank', 'lookahead', 'calibrated', 'joint', 'budget_adapt', 'ddr', 'guarded_joint', 'beam', 'partition_guard', 'shared_input', 'local_repair', 'repair_move', 'region', 'region_gap'})
        ...
        try:
            if arm == 'grain':
                ...
                if 'region' not in queues or queues['region'][0] != current_key:
                    queues['region'] = (current_key, region_candidates(g, best[1], best[2], cores, 'region_gap' in features))
                try:
