...
def check_roadmap(content, filename):
    ...
    font_sizes = [int(value) for value in re.findall(r'fontSize=(\d+)', content)]

    if font_sizes and min(font_sizes) < 10:

        issues.append("CRITICAL: 节点最小字号为 %dpt；最终流程图节点文字不得低于 10pt" % min(font_sizes))
