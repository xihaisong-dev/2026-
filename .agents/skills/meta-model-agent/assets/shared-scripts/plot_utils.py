...
def _save(fig, output):
    ...
    _write_visual_sidecar(fig, output)
...
def _intersection_ratio(first, second):
    ...
    """Return overlap area divided by the smaller bounding-box area."""
    ...
    smaller = min(max(1.0, first.width * first.height), max(1.0, second.width * second.height))

    return overlap / smaller


def _polyline_hits_bbox(points, bbox):

    """Sample display-space line segments against a text or legend box."""
    ...
    for start, end in zip(points[:-1], points[1:]):
        ...
        for fraction in np.linspace(0.0, 1.0, 25):
            ...
            if bbox.x0 + 1 < x < bbox.x1 - 1 and bbox.y0 + 1 < y < bbox.y1 - 1:
...
def _write_visual_sidecar(fig, output):

    """Persist deterministic renderer evidence for final-scale visual QA."""
    ...
    for text in fig.findobj(match=lambda artist: isinstance(artist, Text)):
        ...
        # Isolated significance symbols do not define the readability floor.

        meaningful = len(value) >= 2 or any(ch.isalnum() or '\u4e00' <= ch <= '\u9fff' for ch in value)
        ...
        texts.append({'artist': text, 'text': value, 'fontsize': float(text.get_fontsize()), 'bbox': bbox})
    ...
    for index, first in enumerate(texts):
        ...
        for second in texts[index + 1:]:
            ...
            if _intersection_ratio(first['bbox'], second['bbox']) > 0.02:

                issues.append({

                    'severity': 'critical', 'kind': 'text_text_overlap',

                    'message': f"'{first['text'][:30]}' overlaps '{second['text'][:30]}'"
    ...
    for item in texts:
        ...
        if bbox.x0 < figure_bbox.x0 - 1 or bbox.y0 < figure_bbox.y0 - 1 or bbox.x1 > figure_bbox.x1 + 1 or bbox.y1 > figure_bbox.y1 + 1:

            issues.append({'severity': 'critical', 'kind': 'clipped_text', 'message': f"'{item['text'][:30]}' extends outside the canvas"})
    ...
    for ax in fig.get_axes():
        ...
        for text in ax.texts:
            ...
            if value and text.get_visible():

                try:

                    user_boxes.append((value, text.get_window_extent(renderer=renderer)))
        ...
        data_boxes = list(user_boxes)

        if legend_box is not None:

            data_boxes.append(('legend', legend_box))

        for line in ax.get_lines():

            try:

                points = ax.transData.transform(np.column_stack((line.get_xdata(orig=False), line.get_ydata(orig=False))))

            except Exception:

                continue

            for label, bbox in data_boxes:

                if _polyline_hits_bbox(points, bbox):

                    kind = 'legend_data_overlap' if label == 'legend' else 'text_curve_overlap'

                    issues.append({'severity': 'critical', 'kind': kind, 'message': f"{label[:30]} intersects a plotted line"})
        ...
        for collection in ax.collections:
            ...
            for label, bbox in data_boxes:
                ...
                if any(bbox.x0 + 1 < point[0] < bbox.x1 - 1 and bbox.y0 + 1 < point[1] < bbox.y1 - 1 for point in points):

                    kind = 'legend_data_overlap' if label == 'legend' else 'text_data_overlap'

                    issues.append({'severity': 'critical', 'kind': kind, 'message': f"{label[:30]} covers plotted points"})
    ...
    sidecar = {
        ...
        'asset': str(output).replace('\\', '/'),

        'source_width_pt': round(float(fig.get_size_inches()[0] * 72), 3),

        'source_height_pt': round(float(fig.get_size_inches()[1] * 72), 3),

        'minimum_source_font_pt': round(min((item['fontsize'] for item in texts), default=0.0), 3),

        'text_count': len(texts),

        'critical_count': sum(issue['severity'] == 'critical' for issue in unique_issues),

        'issues': unique_issues,

    }

    sidecar_path = str(output) + '.visual.json'
...
def smart_labels(ax, xs, ys, texts, colors=None, fontsize=12, fontweight='normal',
...
def auto_legend(ax, **kwargs):
    ...
    defaults = dict(frameon=True, edgecolor='#DDD', fontsize=12, fancybox=False, shadow=False)
            ...
            for i in range(data.shape[0]):
                ...
                for j in range(data.shape[1]):
                    ...
                    if j <= i:
                        ...
                        ax.text(j, i, f'{val:{fmt[1:]}}', ha='center', va='center', fontsize=12, color=color)
