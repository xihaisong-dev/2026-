...
ax.set_xticklabels(configs, rotation=18, ha='right', fontsize=12)
...
ax.text(0.98, 0.05, f'Total drop: {scores[0] - scores[-1]:.1f}',
        ...
        transform=ax.transAxes, fontsize=12, color=COLORS['ref_line'],
...
ax1.text(early_stop + 1, ax1.get_ylim()[1] * 0.9, 'Early\nStopping',
         ...
         fontsize=12, color=COLORS['ref_line'], style='italic')
...
ax_inset.set_ylabel('LR (×10⁻³)', fontsize=12)

ax_inset.tick_params(labelsize=5)

ax_inset.set_title('LR Schedule', fontsize=12, fontweight='bold')
...
ax1.set_ylabel('Loss', fontsize=12, color=PALETTE[0])

ax2.set_ylabel('Accuracy (%)', fontsize=12, color=PALETTE[1])
...
for i, cls in enumerate(classes):
    ...
    ax.annotate(cls, xy=(cx, cy), fontsize=12, fontweight='bold', color=PALETTE[i],
...
ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12, markerscale=3, loc='best')
...
ax_heat.set_xticklabels(tokens, rotation=45, ha='right', fontsize=12)

ax_heat.set_yticks(range(n))

ax_heat.set_yticklabels(tokens, fontsize=12)

ax_heat.set_xlabel('Key', fontsize=12)

ax_heat.set_ylabel('Query', fontsize=12)
    ...
    for j in range(n):
        ...
        if attn[i, j] > 0.12:
            ...
            ax_heat.text(j, i, f'{attn[i, j]:.2f}', ha='center', va='center',
                         ...
                         fontsize=12, color='white' if attn[i, j] > 0.25 else 'black')
...
for i, (tok, h) in enumerate(zip(tokens, entropy)):
    ...
    ax_heat.text(n + 0.3, i, f'H={h:.2f}', fontsize=12, va='center', color=COLORS['ref_line'])
...
for j in range(len(datasets)):
    ...
    for i, method in enumerate(methods):
        ...
        ax.text(x[j] + offset, scores[i, j] + stds[i, j] + 0.4,
                ...
                ha='center', fontsize=12, fontweight='bold' if is_best else 'normal',
...
ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12, ncol=len(methods))
...
# 1. 排名数字放在柱子内部顶端：fontsize=12, color='white'（深色柱子）或 color=PALETTE[i]（浅色柱子）
...
for idx, (name, (x_vals, y_mean, y_min, y_max)) in enumerate(params.items()):
    ...
    ax.annotate(f'{y_mean[best_i]:.1f}', xy=(best_i, y_mean[best_i]),
                ...
                xytext=(0, 12), textcoords='offset points', fontsize=12, fontweight='bold',
    ...
    ax.set_xticklabels([str(v) for v in x_vals], fontsize=12)
...
# 3. Cohen's d 标注放在括号中间上方：fontsize=12
...
for sx, sy in saddle_pts:
    ...
    ax.text(sx, sy, sz + 0.15, 'saddle', fontsize=12, color=COLORS['ref_line'], ha='center')

ax.set_xlabel('θ₁', fontsize=12, labelpad=6)

ax.set_ylabel('θ₂', fontsize=12, labelpad=6)

ax.set_zlabel('Loss', fontsize=12, labelpad=6)
...
for i in range(n_samples):
    ...
    for j, method in enumerate(methods):
        ...
        if i == 0:
            ...
            ax.set_title(title, fontsize=12, fontweight=fontw, color=color)
        ...
        if method in ['Input', 'Ground Truth']:
            ...
            if method == 'Ground Truth' and i == 0:
                ...
                ax_err.text(0.5, 0.5, 'Error Map\n(|pred - GT|)', ha='center', va='center',
                            ...
                            fontsize=12, color=COLORS['ref_line'], transform=ax_err.transAxes)
...
ax.text(0.02, 0.02, f'Baseline ({threshold})', fontsize=12, color=COLORS['ref_line'],
...
for i, (name, vals) in enumerate(methods.items()):
    ...
    if name == 'Ours':
        ...
        for j, (ang, val) in enumerate(zip(angles[:-1], vals)):
            ...
            ax.text(ang, val + offset_r, f'{val:.2f}', fontsize=12, ha='center', va='bottom',
...
ax.set_yticklabels(['', '0.4', '0.6', '0.8', '1.0'], fontsize=12, color=COLORS['ref_line'])

ax.legend(loc='best', bbox_to_anchor=(1.35, 1.1), frameon=True, edgecolor=COLORS['grid'], fontsize=12)
...
for i, (cls, pts) in enumerate(zip(classes, all_pts)):
    ...
    ax.text(cx, cy, cz + 0.5, cls, fontsize=12, fontweight='bold', color=PALETTE[i],

            ha='center', va='bottom')

ax.set_xlabel('Dim 1', fontsize=12, labelpad=6)

ax.set_ylabel('Dim 2', fontsize=12, labelpad=6)

ax.set_zlabel('Dim 3', fontsize=12, labelpad=6)
...
ax.text(65, 86, 'iso-efficiency\ncurves', fontsize=12, color=COLORS['grid'], ha='right', style='italic')
...
for i, model in enumerate(models):
    ...
    ax.annotate(model, xy=(flops[i], acc[i]), xytext=offset, textcoords='offset points',
                ...
                fontsize=12, fontweight='bold' if model == 'Ours' else 'normal',
...
ax.text(5, 93.5, 'Better', fontsize=12, fontweight='bold', color=COLORS['up'],
...
ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12, loc='lower right', title='Model Size')
