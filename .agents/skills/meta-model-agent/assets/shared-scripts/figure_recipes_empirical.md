...
for i, (c, lo, hi, lbl, sig, wt) in enumerate(zip(coefs, ci_lo, ci_hi, labels, significant, weights)):
    ...
    ax.text(-0.065, y[i], lbl, ha='right', va='center', fontsize=12, color=COLORS['text'])
    ...
    ax.text(0.085, y[i], f'{c:.3f} [{lo:.3f}, {hi:.3f}]{sig_mark}',
            ...
            ha='left', va='center', fontsize=12, fontfamily='monospace',
...
ax.text(0.0, diamond_y - 1.0, i2_text, ha='center', va='top', fontsize=12,
...
# 5. 变量名长（>10 字符）：fontsize=12，或用缩写+脚注
...
for x_start, x_end, c_base, lbl in [(-5.5, -0.5, _lighten(COLORS['up'], 0.8), 'Pre-Treatment'),
    ...
    ax.text(cx, -0.15, lbl, ha='center', va='bottom', fontsize=12, color=COLORS['ref_line'],
...
ax.text(-0.5, 0.62, 'Treatment', ha='center', va='bottom', fontsize=12,
...
ax.set_xlabel('Period', fontsize=12); ax.set_ylabel('Outcome', fontsize=12)

ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12, loc='upper left')
...
for t_val, c, s in zip(periods, coefs, se):
    ...
    if abs(c) > 1.96 * s:
        ...
        ax.text(t_val, hi + 0.02, star, ha='center', va='bottom',
                ...
                fontsize=12, color=base_color, fontweight='bold')
...
ax2.set_ylabel('Cumulative Effect', fontsize=12, color=PALETTE[1])

ax2.tick_params(axis='y', labelcolor=PALETTE[1])

ax2.legend(loc='upper left', frameon=True, edgecolor=COLORS['grid'], fontsize=12)

ax.set_xlabel('Period Relative to Treatment', fontsize=12)

ax.set_ylabel('Coefficient', fontsize=12); ax.set_xticks(periods)
...
for pval_pct, plabel in [(np.percentile(fake, 5), '5th'), (np.percentile(fake, 95), '95th')]:
    ...
    ax.text(pval_pct, max(yk)*0.92, plabel, ha='center', va='bottom', fontsize=12, color=COLORS['text'],
...
ax.annotate(f'Real = {real:.3f}', xy=(real, max(yk)*0.15),
            ...
            xytext=(real - 0.12, max(yk)*0.65), fontsize=12, fontweight='bold', color=PALETTE[1],
...
ax.set_xlabel('Placebo Coefficients', fontsize=12); ax.set_ylabel('Density', fontsize=12)
...
for i, (grp, data) in enumerate(zip(groups, data_all)):
    ...
    for pct, pct_label in [(25, 'Q1'), (50, 'Median'), (75, 'Q3')]:
        ...
        ax.text(pv, y_center - 0.9, f'{pct_label}\n{pv:.1f}', ha='center', va='top', fontsize=12, color=COLORS['text'])
...
ax.set_yticks([i*2.5 for i in range(len(groups))]); ax.set_yticklabels(groups, fontsize=12)

ax.set_xlabel('Value', fontsize=12); ax.grid(axis='x', alpha=0.15, linestyle='--')
...
for yr, evt in {2015: 'Policy A', 2020: 'Policy B'}.items():
    ...
    ax1.text(yr, gdp.max()+0.3, evt, ha='center', va='bottom', fontsize=12, color=COLORS['down'],
...
ax1.text(0.03, 0.97, f'Pearson r = {corr_val:.3f}', transform=ax1.transAxes, fontsize=12,
...
ax1.set_ylabel('GDP', fontsize=12, color=PALETTE[0])

ax2.set_ylabel('Digital Index', fontsize=12, color=PALETTE[1])

l1, lb1 = ax1.get_legend_handles_labels(); l2, lb2 = ax2.get_legend_handles_labels()

ax1.legend(l1+l2, lb1+lb2, loc='lower right', frameon=True, edgecolor=COLORS['grid'], fontsize=12)
...
ax.text(0.97, 0.97, 'Breusch-Pagan\nχ²=2.14, p=0.143', transform=ax.transAxes, fontsize=12,
...
ax.text(0.03, 0.97, f'Shapiro-Wilk\nW={sw_stat:.3f}\np={sw_p:.3f}', transform=ax.transAxes,
        ...
        fontsize=12, va='top', ha='left', color=COLORS['text'],
...
for idx in np.where(influential)[0]:
    ...
    ax.text(idx, cooks[idx]+0.001, f'#{idx}', fontsize=12, ha='center', color=COLORS['down'], fontweight='bold')
...
ax.text(n*0.97, threshold+0.001, f'4/n={threshold:.3f}', ha='right', va='bottom', fontsize=12, color=COLORS['down'],
...
ax1.annotate(f'Crossover\nMod = {cross_x:.2f}', xy=(cross_x, 0),
             ...
             xytext=(cross_x+0.15, 0.12), fontsize=12, fontweight='bold', color=PALETTE[1],
...
ax1.legend(fontsize=12, frameon=True, edgecolor=COLORS['grid']); ax1.grid(alpha=0.15, linestyle='--')
...
ax2.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12); ax2.grid(alpha=0.15, linestyle='--')
...
for gn, gd in groups.items():
    ...
    ax.text(-0.065, yp, f'■ {gn}', ha='left', va='center', fontsize=12, fontweight='bold', color=gc)
    ...
    for lb, c, s, sig, wt in gd['items']:
        ...
        ax.text(-0.065, yp, f'  {lb}', ha='left', va='center', fontsize=12, color=COLORS['text'])

        ax.text(0.065, yp, f'{wt:.1f}%', ha='right', va='center', fontsize=12, color=COLORS['ref_line'])
    ...
    ax.text(-0.065, yp, '  Pooled', ha='left', va='center', fontsize=12, fontweight='bold', color=gc)

    ax.text(0.065, yp, f'I²={gd["I2"]:.1f}%, Q p={gd["Q_p"]:.3f}', ha='right', va='center',

            fontsize=12, color=COLORS['ref_line'], fontstyle='italic')
...
# 4. 数值标签统一放在 CI 右端外侧：fontsize=12（比普通森林图更小）
...
for qi, ci, si in zip(q, c, se):
    ...
    if abs(ci) > 1.96*si:
        ...
        ax.text(qi, ci+1.96*si+0.015, star, ha='center', va='bottom', fontsize=12, color=PALETTE[0], fontweight='bold')

ax.annotate(f'τ=0.9: {c[-1]:.3f}', xy=(q[-1], c[-1]),

            xytext=(q[-1]-0.15, c[-1]+0.12), fontsize=12, color=PALETTE[0],
...
ax.set_xlabel('Quantile (τ)', fontsize=12); ax.set_ylabel('Coefficient', fontsize=12)

ax.set_xticks(q); ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12, loc='upper left')
...
for i in range(len(covs)):
    ...
    ax.text((before[i]+after[i])/2, y[i]+0.25, f'↓{improvement:.0f}%', ha='center', va='bottom',
            ...
            fontsize=12, color=COLORS['up'], fontweight='bold')
...
ax.set_yticks(y); ax.set_yticklabels(covs, fontsize=12)

ax.set_xlabel('Standardized Mean Difference', fontsize=12); ax.invert_yaxis()
...
ax.text(0.97, 0.03, f'Avg |SMD| Before: {avg_b:.3f}\nAvg |SMD| After: {avg_a:.3f}\nReduction: {(1-avg_a/avg_b)*100:.1f}%',
        ...
        transform=ax.transAxes, fontsize=12, va='bottom', ha='right', color=COLORS['text'],

        bbox=dict(boxstyle='square,pad=0.4', facecolor=COLORS['bg_box'], edgecolor=COLORS['grid'], alpha=0.95))

ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12, loc='best')
...
ax.text(split_point/2, actual.max()+2, 'Training', ha='center', fontsize=12, color=COLORS['text'],

        bbox=dict(boxstyle='square,pad=0.25', facecolor='white', edgecolor=COLORS['grid'], alpha=0.9))

ax.text((split_point+100)/2, actual.max()+2, 'Testing', ha='center', fontsize=12, color=COLORS['text'],
...
ax.text(0.02, 0.03, f'Test Metrics:\nRMSE = {rmse:.3f}\nMAE  = {mae:.3f}\nR²   = {r2:.3f}',
        ...
        transform=ax.transAxes, fontsize=12, va='bottom', ha='left', color=COLORS['text'], family='monospace',
...
ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12); ax.grid(alpha=0.15, linestyle='--')
...
ax_hist.set_xlabel('Density', fontsize=12); ax_hist.set_yticklabels([])
...
for j in range(data.shape[1]):
    ...
    for i in range(data.shape[0]):
        ...
        ax.text(j, i, f'{data[i,j]:.2f}{rank_str}', ha='center', va='center',
                ...
                fontsize=12, fontweight=w, color=txt_color)
...
ax.set_xticks(range(len(metrics))); ax.set_xticklabels(metrics, fontsize=12)

ax.set_yticks(range(len(models))); ax.set_yticklabels(models, fontsize=12)

cbar = fig.colorbar(im, ax=ax, shrink=0.7, pad=0.02)

cbar.set_label('Normalized Score\n(1=Best, 0=Worst)', fontsize=12)
...
for i in range(len(models)):
    ...
    ax.text(len(metrics)+0.1, i, f'Avg: {avg_score[sort_idx[i]]:.2f}', ha='left', va='center', fontsize=12, color=COLORS['text'])
...
for i, (model, err) in enumerate(zip(models, errors)):
    ...
    if len(outliers) > 0:
        ...
        for ov in outliers[:3]:
            ...
            ax.text(ov, y_pos+0.6, f'{ov:.1f}', ha='center', va='bottom', fontsize=12, color=COLORS['down'], fontweight='bold')

    t_stat, p_val = ttest_1samp(err, 0)

    sig_str = '***' if p_val < 0.001 else ('**' if p_val < 0.01 else ('*' if p_val < 0.05 else 'ns'))

    ax.text(err.max()+1, y_pos, f't={t_stat:.2f} {sig_str}', ha='left', va='center', fontsize=12, color=COLORS['text'],
...
ax.text(0, len(models)*1.5-0.3, 'Zero Error', ha='center', va='bottom', fontsize=12, color=COLORS['down'],

        bbox=dict(boxstyle='square,pad=0.2', facecolor=COLORS['bg_box'], edgecolor=COLORS['down'], alpha=0.9))

ax.set_yticks([i*1.5 for i in range(len(models))]); ax.set_yticklabels(models, fontsize=12)

ax.set_xlabel('Prediction Error', fontsize=12); ax.grid(axis='x', alpha=0.15, linestyle='--')
...
ax.text(12.3, threshold, 'Acceptable\nThreshold', ha='left', va='center', fontsize=12, color=COLORS['up'],
...
for i, (name, acc) in enumerate(models_data.items()):
    ...
    if name in ['Ours', 'ARIMA']:
        ...
        ax.annotate(f'Decay: {decay_rate:.1f}%', xy=(steps[-1], acc[-1]),
                    ...
                    xytext=(steps[-1]-2.5, acc[-1]-0.04), fontsize=12, color=PALETTE[i],
...
ax.set_xlabel('Prediction Horizon (steps ahead)', fontsize=12); ax.set_ylabel('R² Score', fontsize=12)

ax.set_xticks(steps); ax.legend(frameon=True, edgecolor=COLORS['grid'], fontsize=12, loc='lower left', ncol=2)
...
ax.set_xlabel('标准化变量 (z)', fontsize=12)
...
# 4. 象限标签放在四角：fontsize=12, alpha=0.6
...
for _, row in gdf[gdf['lisa_cluster'] != 'NS'].iterrows():
    ...
    ax.text(c.x, c.y, short, ha='center', va='center', fontsize=12,
...
ax.legend(handles=legend_elements, loc='lower left', fontsize=12,
          ...
          title=f'LISA 聚类 (p < {sig_level})', title_fontsize=12)
...
for i, (x, y) in enumerate(zip(n_controls, coefs)):
    ...
    ax.text(x, y + 0.002, f'{y:.3f}', ha='center', va='bottom', fontsize=12,
...
ax.set_xticklabels(specs, fontsize=12, rotation=30, ha='right')

ax.set_ylabel('核心系数估计值', fontsize=12)

ax.set_xlabel('模型设定（逐步加入控制变量）', fontsize=12)

ax.legend(fontsize=12, frameon=True, edgecolor=COLORS['grid'], loc='best')
...
ax1.set_ylabel('方差贡献比例 (%)', fontsize=12)

ax1.set_ylim(0, 100)

ax1.legend(fontsize=12, frameon=True, edgecolor=COLORS['grid'], loc='center right')

ax1.text(-0.05, 1.06, '(a) 完整方差分解', transform=ax1.transAxes, fontsize=12, fontweight='bold')
...
ax2.text(periods[-1] + 0.3, var2[-1], f'{var2[-1]:.1f}%', va='center', fontsize=12,

         color=PALETTE[1], fontweight='bold')

ax2.text(periods[-1] + 0.3, var2[-1] + var3[-1], f'{var3[-1]:.1f}%', va='center', fontsize=12,

         color=PALETTE[2], fontweight='bold')

ax2.set_ylabel('方差贡献比例 (%)', fontsize=12)

ax2.set_xlabel('预测期', fontsize=12)

ax2.legend(fontsize=12, frameon=True, edgecolor=COLORS['grid'], loc='upper left')

ax2.text(-0.05, 1.06, '(b) 非自身冲击成分（放大）', transform=ax2.transAxes, fontsize=12, fontweight='bold')
...
for idx, (title, data) in enumerate(responses.items()):
    ...
    ax.text(0.5, 1.05, title, transform=ax.transAxes, fontsize=12, fontweight='bold', ha='center')
...
axes[-2].set_xlabel('期数', fontsize=12)

axes[-1].set_xlabel('期数', fontsize=12)
