...
def check_figure_script(filepath: str) -> list:
    ...
    # 9.5 明显过小的硬编码字号。最终有效字号仍由 rendered_visual_audit 复核。

    for match in re.finditer(r"fontsize\s*=\s*([0-9]+(?:\.[0-9]+)?)", code):
        ...
        if value < 9.0:
            ...
            issues.append(f"L{line}: fontsize={value:g}pt 低于源图绝对下限 9pt；常规 0.72\\linewidth 嵌入建议源字号至少 12pt")
