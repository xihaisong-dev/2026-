...
\subsubsection{真实张量的 FIFO 生命周期}
图\ref{fig:q3-fifo-events}摘取第39图最终五核方案中张量1000000005的官方缓存事件。该4 KiB张量在39485周期填入，在51610周期命中；命中不改变FIFO先后，随后在63967周期被新填充挤出，77594周期的再次读取仍然未命中，直到77801周期重新填入。图中节点等距排列只表达事件顺序，数值是实际发生周期。该例说明“曾经命中”不等于后续一直驻留，也解释了缓存邻域为何必须结合读取时刻和淘汰事件，而不能只统计复用次数。

\begin{figure}[!htbp]\centering
\includegraphics[width=0.92\linewidth,height=0.70\textheight,keepaspectratio]{../图表/runs/20260926-A-six-figures/fig_q3_fifo_events.pdf}
\caption{第39图一个只读张量的真实FIFO事件链}\label{fig:q3-fifo-events}
\end{figure}
