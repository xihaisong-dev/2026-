...
先填写 `协作/分工.md`，再领取 Issue。目录负责人负责接口与审阅，其他人可以协助；修改别人的主责目录前，先在任务里明确范围和接收人。
...
- 多人协助论文时拆分 `论文/章节/`，由 W 统一修改入口、参考文献和版式。
...
当前阶段为 DISCOVERY / ready，尚未启动。缺少赛题或当届官方模板时，begin 会阻断，不能手改状态绕过。阶段和任务状态独立管理；详细操作见 `协作/工作流.md`。团队 clone 后不用重新 init；由 O 串行维护唯一阶段状态。
