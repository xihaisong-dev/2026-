...
def generate_candidates(graph, num_cores, problem=1, budget='standard'):
    ...
    if not large or budget == 'standard':
        separate = problem == 1
        add('component_pipe_lpt',_component_plan(m,n,'pipe_lpt',not separate))
        add('component_comm',_component_plan(m,n,'comm',not separate))
