...
def stitch_plans(m, n, graph):
    ...
    if nodes <= 12000:
        # weighted_window never won a problem-1 case. Both critical-cut
        # settings did, once each, so both stay.
        for balance, scale in ((0.16, 1.0), (0.06, 0.35)):
            _try('critical_cut_b%s_s%s' % (balance, scale),
                 ...
                 out, seen)
        # win_w7 is the case_067 winner. Widths 5, 14 and 23 never won.
        if depth + 1 >= 14:
            _try('win_w7', lambda: _depth_window(m, n, 7), out, seen)
    ...
    # Pack consecutive weak components. Size 3 is the local-search seed on
    # small graphs. Sizes 4 and 8 win only below 12000 nodes; above that each
    # group is a slow evaluation and is not selected. Sizes 5 and 6 never won.
    if 3 <= components <= 200:
        sizes = []
        if nodes <= 8000:
            sizes.append(3)
        if nodes <= 12000:
            sizes.extend((4, 8))
        if sizes:
            for name, plan in _component_groups(m, n, tuple(sizes)):
